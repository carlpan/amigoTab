...
class DepartmentController extends Controller
{
    ...
    /*
    private function getAllData($year, $month, $department, $num) {
        $data = array();
        $match = ['year_id' => $year, 'month_id' => $month, 'department_id' => $department];

        // queries to get lists
        $name = DB::table('sales_items')->where($match)->take($num)->lists('name');
        $unit = DB::table('sales_items')->where($match)->take($num)->lists('quantity_unit');
        $price = DB::table('sales_items')->where($match)->take($num)->lists('price');
        $quantity = DB::table('sales_items')->where($match)->take($num)->lists('quantity');
        $revenue = DB::table('sales_items')->where($match)->take($num)->lists('revenue');

        // build up array with key as name and value as ['unit', 'price', 'quantity', 'revenue']
        for ($index = 0; $index < count($name); $index++) {
            $data[$name[$index]] = [$unit[$index], $price[$index], $quantity[$index], $revenue[$index]];
        }

        return $data;
    }*/
    private function getAllData($year, $month, $department) {
        $data = array();
        $match = ['year_id' => $year, 'month_id' => $month, 'department_id' => $department];

        // queries to get lists
        $name = DB::table('sales_items')->where($match)->lists('name');
        $unit = DB::table('sales_items')->where($match)->lists('quantity_unit');
        $price = DB::table('sales_items')->where($match)->lists('price');
        $quantity = DB::table('sales_items')->where($match)->lists('quantity');
        $revenue = DB::table('sales_items')->where($match)->lists('revenue');
    ...
    public function showAppetizers() {
        ...
        $all = $this->getAllData($year, $month, $department);
